...
| O.A.2 | Cabler Sure Feet / Sprint dans `movement.ts` | Engine | S | [ ] | `mechanics/movement.ts` hardcode `player.skills.includes('sure-feet')`. Migrer vers `collectModifiers(player, 'on-gfi', context)` + `getGfiCap(player)`. Skill registry deja en place. Tests existent (follow-up-b01.md). |
| O.A.3 | Cabler Horns / Pile Driver dans `blocking.ts` | Engine | S | [ ] | `mechanics/blocking.ts` : `collectModifiers(attacker, 'on-block-attacker', { isBlitz })` au lieu de `hasSkill('horns')`. Verifie que Pile Driver active passif "insta-stun" via registry trigger `on-block-result`. |
| O.A.4 | Cabler Stunty / Sneaky Git | Engine | S | [ ] | `mechanics/blocking.ts:141` Stunty (-1 AV) + `mechanics/foul.ts` Sneaky Git (foul reroll). Migrer ces 2 dernier hardcodes vers registry. |
| O.A.5 | Resolve Perfect Defence kickoff event | Engine | M | [ ] | `mechanics/kickoff-resolution.ts` : ajout case `'perfect-defence'` qui expose `pendingKickoffDecision: { type: 'perfect-defence', team: receivingTeam }`. UI permet repositionnement LOS-compliant. Test : kickoff event 4 → reorg autorise → game state coherent. |
| O.A.6 | Add Illegal Procedure check | Engine | M | [ ] | `mechanics/referee.ts` : lors de foul action, valider count(assistants) ≤ legale (BB officiel Season 2/3). Sinon turnover auto + log event ILLEGAL_PROCEDURE. Test : `foul.test.ts` ajoute scenario 4 assistants → turnover. |
| O.A.7 | Wire Diving Tackle dans dodge sequence | Engine | M | [ ] | `mechanics/movement.ts` (`processDodge`) : appel `collectModifiers(defender, 'on-opponent-dodge', context)`. Le defender adjacent peut declarer DT → ajoute -2 au dodge target. Skill registry deja enregistre. |
...
| O.B.2 | Email verification (optionnel, light) | Backend | M | [ ] | Ajoute champ `User.emailVerifiedAt`. Sur signup, envoie email avec token (utilise service mail existant) → `/verify-email?token=...` → set `emailVerifiedAt`. Pas bloquant pour la connexion (warning bannier seulement). Differable en O.B.2.5 si pas de SMTP configure. |
