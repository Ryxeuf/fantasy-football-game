...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a (foundation mensualisation) — helpers `nuffle-cup-monthly.ts` + champs `monthlyYear/Month` + migration. S27.1b (endpoint listing) — service `cup-monthly-listing.ts` + route publique `GET /cup/monthly`. S27.1c (UI calendrier) — page publique `/cups/monthly` (`apps/web/app/cups/monthly/page.tsx`) consomme `GET /cup/monthly` : grille responsive (cards par cup avec nom, libelle MM/YYYY, badge statut localise FR/EN, lien deep `/cups/{id}`), empty/error states, traductions FR/EN ajoutees. 7 tests UI (loading, listing, ordre, deeplink, empty, error, endpoint appele). Restent : creation cup mensuelle (admin) + bracket visuel UI + match-of-week + badge profil "Champion Nuffle Cup".** |
