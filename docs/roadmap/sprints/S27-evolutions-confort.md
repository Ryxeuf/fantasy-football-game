...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a-e (foundation, listing, UI calendrier, badge backend + UI). S27.1f (match-of-the-week foundation) — champs `featuredAt: DateTime?` + `featuredNote: String?` sur `LocalMatch` + index `featuredAt`, migration `20260504100000_add_local_match_featured`, service `services/match-of-the-week.ts` (`getCurrentMatchOfTheWeek()`) qui retourne le LocalMatch le plus recemment featured (filtre `isPublic=true` defense en profondeur), nouvelle route publique `GET /cup/match-of-the-week` (montee avant `/:id`). 5 tests service. Restent : endpoint admin pick + UI badge "Match du moment" + creation cup mensuelle (admin) + bracket visuel.** |
