...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a (foundation mensualisation) — helpers + champs Cup + migration. S27.1b (endpoint listing) — service + route `GET /cup/monthly`. S27.1c (UI calendrier) — page publique `/cups/monthly`. S27.1d (badge champion backend) — service `services/cup-championships.ts` (`getCoachCupChampionships(userId)`) qui parcourt les Cup `status='terminee'/'archivee'` avec `monthlyYear/Month NOT NULL`, calcule le 1er via `computeCupStandings`, formate "Champion Nuffle Cup {Mois} {YYYY}", ordre `monthlyYear DESC, monthlyMonth DESC`, defense en profondeur (slot invalide / standings vides / participants vides ignores). `GET /coach/:slug` enrichi : `cupChampionships` ajoute en parallele de `championships`/`achievements`/`recentTeams` via `Promise.all`. 8 tests service + 2 tests handler. Restent : UI badge cup sur `/coach/[slug]` + creation cup mensuelle (admin) + bracket visuel + match-of-week.** |
