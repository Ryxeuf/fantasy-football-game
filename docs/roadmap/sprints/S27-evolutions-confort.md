...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a-g (foundation cup mensuelle, endpoint listing, UI calendrier, badge champion backend + UI, match-of-the-week lecture/admin pick). S27.1h (UI banner match-of-the-week) — composant `apps/web/app/cups/MatchOfTheWeekBanner.tsx` consomme `GET /cup/match-of-the-week` (degradation gracieuse : rend null en silence si fetch en cours, aucun match featured ou API en erreur), branche en haut de `/cups/monthly/page.tsx` au-dessus du calendrier (etoile ⭐ + teams + score + note + lien vers `/cups/{cupId}`), traductions FR/EN. 5 tests UI banner. Restent : creation cup mensuelle (admin) + bracket visuel.** |
