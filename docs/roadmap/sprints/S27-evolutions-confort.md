...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a-f (foundation cup mensuelle, endpoint listing, UI calendrier, badge champion backend + UI, match-of-the-week lecture). S27.1g (admin pick match-of-the-week) — service `setMatchOfTheWeek({matchId, note})` defensif (refuse match introuvable / non public / non completed), Zod `setMatchOfTheWeekSchema` (note max 280 chars), route `POST /cup/match-of-the-week/:matchId` protegee `authUser` + `hasRole(admin)` inline, set `featuredAt=now()` et `featuredNote`. 5 nouveaux tests service. Restent : UI badge "Match du moment" + creation cup mensuelle (admin) + bracket visuel.** |
