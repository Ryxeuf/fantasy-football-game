...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a (foundation) + S27.1b (endpoint listing) + S27.1c (UI calendrier) + S27.1d (badge backend). S27.1e (badge UI) — type web `CoachCupChampionship` + `cupChampionships?` ajoute au DTO `CoachPublicProfile`, nouveau composant `apps/web/app/coach/[slug]/CoachCupChampionshipsBanner.tsx` (rend null si vide, sinon liste de badges gold/bronze avec emoji 🏆 trophee + label + sous-texte MM/YYYY + lien deep `/cups/{cupId}`), branche dans `page.tsx` apres `CoachChampionshipsBanner` (ligues thematiques) avant `CoachEloChart`. 6 tests UI banner cup. Restent : creation cup mensuelle (admin) + bracket visuel + match-of-week.** |
