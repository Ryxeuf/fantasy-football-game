...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a (mensualisation foundation) — service `services/nuffle-cup-monthly.ts` (helpers `MONTH_LABELS_FR`, `getMonthLabelFr`, `isValidMonthlyCupSlot`, `formatMonthlyNuffleCupName`, `formatMonthlyNuffleCupChampionLabel`), champs `monthlyYear: Int?` + `monthlyMonth: Int?` sur `Cup` + index `(monthlyYear, monthlyMonth)`, migration `20260504000000_add_cup_monthly_slot`. 12 tests. S27.1b (endpoint listing) — service `services/cup-monthly-listing.ts` (`listMonthlyCups({year?, month?, limit?, offset?})`, validation defensive year > 0 et month dans [1,12], ordre `monthlyYear DESC, monthlyMonth DESC`, cap limit 100), Zod `listMonthlyCupsQuerySchema`, route publique `GET /cup/monthly` (montee avant `/:id` pour eviter le pattern matching). 7 tests service. Restent : creation cup mensuelle (admin) + bracket visuel UI + match-of-week + badge profil "Champion Nuffle Cup".** |
