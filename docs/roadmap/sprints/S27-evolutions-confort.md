...
| S27.2 | Mobile parite : Action Picker Throw Team-Mate + flow 2-clics | AMELIO | M | [~] | `apps/mobile/app/play/[id].tsx:390`. Refrappe ce qui a ete fait en S23 sur web (commit `28c31e4`). Backend deja pret (B2.4). Idem pour les autres popups d'action si lacunes detectees. **EN COURS : S27.2.1 (port helper foundation) — `apps/mobile/lib/throw-team-mate-click.ts` (fonction pure `processThrowTeamMateClick(input): ThrowTeamMateClickResult`), 4 etats discriminants (`inactive`, `noop`, `selectThrown`, `submit`) pour que le caller decide. Logique flow 2-clics (Phase 1 selection coequipier lancable, Phase 2 selection case cible). 6 tests unitaires. Restent : integration dans `app/play/[id].tsx` (handler tap + indicateur fixe + bouton annuler) + verification autres popups d'action mobile.** |
