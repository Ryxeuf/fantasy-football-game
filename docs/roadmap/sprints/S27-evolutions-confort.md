...
|---|-------|-----|--------|--------|--------|
| S27.1 | Compétitions esport mensuelles "Nuffle Cup" + bracket visuel + match-of-week | EVO | L | [~] | Infra Cup deja en place (`prisma/Cup`, standings, matchmaking). Lancer : 1 cup mensuelle programee, bracket visuel `/cups/{slug}`, page "match-of-the-week" (pick admin), badge profil "Champion Nuffle Cup {date}". **Version A : 100% numerique** (trophees + badge + Discord match-of-week). Pas de prix physique pour eviter logistique / fiscal. Ouvert a evolution selon retours communautaires. **EN COURS : S27.1a-h (foundation, listing, UI calendrier, badge champion, match-of-the-week pipeline complet). S27.1i (creation cup mensuelle admin) — `createCupSchema` Zod etendu (`monthlyYear` + `monthlyMonth` couple obligatoire, validation entiers/bornes), handler `POST /cup` propage les champs au create Prisma uniquement quand l'utilisateur a `hasRole(admin)` (les coachs reguliers peuvent toujours creer des cups privees mais pas de cup mensuelle canonique). Erreur 403 si non-admin tente de fournir le couple. 6 nouveaux tests Zod. Restent : bracket visuel `/cups/{slug}` UI.** |
