...
| S26.4 | Systeme d'amis : invitation par `@username` (pas userId) | AMELIO | M | [~] | `apps/server/src/services/friendship.ts:14`. `sendFriendRequest(targetUserId)` actuel = userId interne, friction max. Ajouter resolveur `@username -> userId` + autocomplete UI. **EN COURS : S26.4a (service `findUserByCoachName`), S26.4b (`POST /friends` accepte `{ username }` exclusif a `{ receiverId }` via Zod oneOf + helper `resolveReceiverIdFromBody`).** |
