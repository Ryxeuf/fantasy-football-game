...
| S25.5 | Adopter `ApiResponse<T>` sur les 75% routes restantes | AMELIO | L | [~] | Std error handling. Aujourd'hui ~25/200+ routes l'utilisent (`utils/api-response.ts` cree pour O.6). Refactor `match.ts`, `team.ts`, `league.ts` en priorite. **Avancement** : S25.5a — `league.ts` migre cote erreurs (sendError, non-breaking client). S25.5b/c/d — success paths league + match.ts (1606 lignes) + team.ts (2151 lignes) restent a faire avec coordination front. |
