...
// Endpoint pour obtenir les Star Players disponibles pour une equipe (S25.5t — ApiResponse<T>)
  ...
  try {
    ...
    const currentPlayersCost = team.players.reduce(
      (total: number, player: any) =>
        total + getPlayerCost(player.position, team.roster, teamRuleset),
      0,
    );

    const currentStarPlayersCost = team.starPlayers.reduce(
      (total: number, sp: any) => total + sp.cost,
      0,
    );
