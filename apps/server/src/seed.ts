...
async function main() {
  ...
  // Catalogues de règles spéciales et de Ligues régionales : les deux tables
  // existaient mais restaient vides, le produit servait les libellés
  // compilés. Create-only : une description corrigée en admin survit.
  const rulesRes = await syncTeamRules({ write: true });
  serverLog.log(
    `✅ Règles spéciales: ${rulesRes.specialRules.created} créées, ` +
  ...
  // Données de DÉMO (équipes Skaven/Lizardmen pour CHAQUE utilisateur, coachs
  // et équipes type par roster, coupe + match local de test). Jamais en
  // production : le seed y a déjà créé ~110 équipes « X-Skavens /
  // X-Lizardmen » pour de vrais coachs (budget en po au lieu de kpo).
  // `SEED_DEMO_DATA=1` force leur création (environnement de recette).
  const seedDemoData =
    process.env.NODE_ENV !== "production" || process.env.SEED_DEMO_DATA === "1";
  if (!seedDemoData) {
    serverLog.log("⏭️  Données de démo ignorées (production)");
  }
