  ...
  it('devrait reproduire le bug où A2 ne peut plus rien faire après un blitz', () => {
    ...
    const initialState = {
      ...
      players: state.players.map(p => {
        if (p.id === 'A1') return { ...p, pos: { x: 12, y: 7 }, stunned: false, pm: 6, hasBall: false };
        if (p.id === 'A2') return { ...p, pos: { x: 16, y: 9 }, stunned: false, pm: 6, hasBall: false };
        if (p.id === 'B1') return { ...p, pos: { x: 13, y: 7 }, stunned: false, pm: 6, hasBall: true };
        if (p.id === 'B2') return { ...p, pos: { x: 18, y: 9 }, stunned: false, pm: 6, hasBall: false };
        return p;
      }),
      currentPlayer: 'A',
    };
    ...
    const blitzMoves = movesBeforeBlitz.filter(m => m.type === 'BLITZ' && m.playerId === 'A2');
    ...
    const continueMoves = movesAfterBlitz.filter(m => m.type === 'MOVE' && m.playerId === 'A2');
    
    console.log('Actions disponibles après blitz:', movesAfterBlitz.map(m => ({ type: m.type, playerId: m.type === 'MOVE' ? m.playerId : undefined })));
