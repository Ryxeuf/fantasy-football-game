...
describe("Play/[id] header hydration", () => {
  beforeEach(() => {
    ...
    global.fetch = vi.fn(async (url: string, init?: any) => {
      if (url.endsWith("/match/details")) {
        ...
        return new Response(JSON.stringify({ error: "x-match-token requis" }), {
          status: 401,
        });
      }
  ...
  it("affiche les valeurs du serveur (score/turn/half/noms) et non la démo", async () => {
    render(
      React.createElement(PlayByIdPage as any, {
        params: { id: "cmfy5qrjm0000qpazp4o4vzpi" },
      }),
    );

    await waitFor(() =>
      expect(screen.getByTestId("left-name").textContent).toContain("Skavens"),
    );
    await waitFor(() =>
      expect(screen.getByTestId("right-name").textContent).toContain(
        "Hommes-Lézards",
      ),
    );
    await waitFor(() =>
      expect(screen.getByTestId("score").textContent).toBe("1-0"),
    );
    await waitFor(() =>
      expect(screen.getByTestId("turn").textContent).toBe("T3 H1"),
    );
  });
});
