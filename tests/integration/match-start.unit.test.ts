...
vi.mock("@bb/game-engine", () => ({
  ...
  setupPreMatchWithTeams: (
    ...
    isTurnover: false,
    dugouts: {
      teamA: { teamId: "A", zones: { reserves: { players: [] } } },
      teamB: { teamId: "B", zones: { reserves: { players: [] } } },
    },
    ...
    gameLog: [
      {
        type: "info",
        message: `Phase pré-match - ${teamAName} vs ${teamBName} - Les joueurs sont en réserves`,
      },
    ],
  }),
}));
...
function makePrismaMock(opts: {
  ...
  const turns: any[] = (opts.usersAccepted || []).map((u, i) => ({
    matchId: "m1",
    number: i + 1,
    payload: { type: "accept", userId: u },
  }));
  return {
    match: {
    ...
    teamSelection: {
      findFirst: async (_: any) => ({
        id: "sel",
        userId: _.where.userId,
        matchId: _.where.matchId,
      }),
...
describe("acceptAndMaybeStartMatch (unitaire, sans DB)", () => {
  it("attend le deuxième joueur si un seul accepte", async () => {
    ...
    const r = await acceptAndMaybeStartMatch(prisma as any, {
      matchId: "m1",
      userId: "u1",
    });
    expect(r.ok).toBe(true);
    expect(
      r.status === "waiting_other_player" ||
        r.status === "waiting_other_accept",
    ).toBe(true);
  });
