...
const TEST_DATABASE_URL = process.env.TEST_DATABASE_URL || 'file:memdb1?mode=memory&cache=shared';
...
export async function setup() {
  ...
  // Vérifier si up
  try {
    const res = await fetch(`http://localhost:${API_PORT}/health`).catch(() => null as any);
    if (res && res.ok) {
  ...
  serverProc = spawn('pnpm', ['run', 'dev:nowatch'], {
    ...
    env: { ...process.env, API_PORT, BGIO_PORT, TEST_SQLITE: '1', TEST_DATABASE_URL },
  });
  (globalThis as any).__serverProc = serverProc;
  await wait(1200);
  // Reset DB avant tests
  await fetch(`http://localhost:${API_PORT}/__test/reset`, { method: 'POST' }).catch(() => null);
  (globalThis as any).__serverStarted = true;
}
