...
describe('Tests des dés de blocage - Vérification complète', () => {
  ...
  describe('Vérification des faces de dés', () => {
    it('devrait avoir exactement 5 faces de dés de blocage (6 faces avec Push Back dupliqué)', () => {
      const expectedFaces: BlockResult[] = ['PLAYER_DOWN', 'BOTH_DOWN', 'PUSH_BACK', 'STUMBLE', 'POW'];
  ...
  describe('Tests d\'intégration avec le jeu', () => {
    it('devrait exécuter un blocage complet avec tous les résultats possibles', () => {
      ...
      const testResults: BlockResult[] = ['PLAYER_DOWN', 'BOTH_DOWN', 'PUSH_BACK', 'STUMBLE', 'POW'];
      ...
      for (const testResult of testResults) {
        ...
        const blockLogs = resultState.gameLog.filter(log => 
          log.type === 'dice' && log.message.includes('Blocage:')
        );
  ...
  describe('Tests de conformité avec les règles', () => {
    it('devrait respecter les règles de Blood Bowl pour les résultats', () => {
      ...
      const testCases = [
        ...
        { result: 'STUMBLE', expected: 'Si la cible utilise Dodge, cela devient Push ; sinon, c\'est POW!', probability: '1/6' },
      ...
      for (const testCase of testCases) {
        ...
        expect(['PLAYER_DOWN', 'BOTH_DOWN', 'PUSH_BACK', 'STUMBLE', 'POW']).toContain(testCase.result);
