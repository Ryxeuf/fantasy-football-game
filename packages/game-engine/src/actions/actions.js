 ...
 */
import { inBounds, samePos, requiresDodgeRoll, calculateDodgeModifiers, calculatePickupModifiers, } from '../mechanics/movement';
import { rollBlockDiceManyWithRolls, } from '../utils/dice';
import { performDodgeRollWithNotification, performPickupRollWithNotification, performArmorRollWithNotification, rollBlockDiceWithNotification, rollBlockDiceManyWithNotification, } from '../utils/dice-notifications';
...
import { canBlock, canBlitz, calculateOffensiveAssists, calculateDefensiveAssists, calculateBlockDiceCount, getBlockDiceChooser, resolveBlockResult, } from '../mechanics/blocking';
import { hasPlayerActed, canPlayerMove, canPlayerContinueMoving, setPlayerAction, checkPlayerTurnEnd, shouldAutoEndTurn, handlePlayerSwitch, getPlayerAction, incrementTeamBlitzCount, advanceHalfIfNeeded, } from '../core/game-state';
/**
...
export function getLegalMoves(state) {
    ...
    const myPlayers = state.players.filter(p => p.team === team && (canPlayerMove(state, p.id) || canPlayerContinueMoving(state, p.id)));
    const occ = new Map();
 ...
 */
function getAdjacentOpponents(state, position, team) {
    const opponents = [];
    const dirs = [
        { x: 1, y: 0 },
        { x: -1, y: 0 },
    ...
    for (const dir of dirs) {
        ...
        const opponent = state.players.find(p => p.team !== team && p.pos.x === checkPos.x && p.pos.y === checkPos.y && !p.stunned);
        if (opponent) {
 ...
 */
export function applyMove(state, move, rng) {
...
function handleEndTurn(state) {
    ...
    const newState = {
        ...
        playerActions: new Map(), // Réinitialiser les actions
        teamBlitzCount: new Map(), // Réinitialiser les compteurs de blitz
    };
    // Log du changement de tour
    const turnLogEntry = createLogEntry('action', `Fin du tour - ${newState.currentPlayer === 'A' ? newState.teamNames.teamA : newState.teamNames.teamB} joue maintenant`, undefined, newState.currentPlayer);
 ...
 */
function handleMove(state, move, rng) {
    ...
    const legal = getLegalMoves(newState).some(m => m.type === 'MOVE' && m.playerId === move.playerId && samePos(m.to, move.to));
    if (!legal)
        return newState;
 ...
 */
function handleDodgeRoll(state, player, from, to, rng, idx) {
    ...
    const logEntry = createLogEntry('dice', `Jet d'esquive: ${dodgeResult.diceRoll}/${dodgeResult.targetNumber} ${dodgeResult.success ? '✓' : '✗'}`, player.id, player.team, {
    ...
    if (dodgeResult.success) {
        ...
        // Log du jet d'armure
        const armorLogEntry = createLogEntry('dice', `Jet d'armure: ${armorResult.diceRoll}/${armorResult.targetNumber} ${armorResult.success ? '✓' : '✗'}`, next.players[idx].id, next.players[idx].team, {
 ...
 */
function handleNormalMove(state, player, from, to, rng, idx) {
    ...
    next = checkPlayerTurnEnd(next, player.id);
    // Log du mouvement
    const moveLogEntry = createLogEntry('action', `Mouvement vers (${to.x}, ${to.y})`, player.id, player.team);
 ...
 */
function handleBallPickup(state, player, rng, idx) {
    ...
    const pickupLogEntry = createLogEntry('dice', `Jet de pickup: ${pickupResult.diceRoll}/${pickupResult.targetNumber} ${pickupResult.success ? '✓' : '✗'}`, player.id, player.team, {
    ...
    if (pickupResult.success) {
        ...
        const successLogEntry = createLogEntry('action', `Ballon ramassé avec succès`, player.id, player.team);
        ...
        const failLogEntry = createLogEntry('turnover', `Échec du ramassage - Turnover`, player.id, player.team);
 ...
 */
function handleDodge(state, move, rng) {
    ...
    next.lastDiceResult = dodgeResult;
    // Log du jet d'esquive
    const dodgeLogEntry = createLogEntry('dice', `Jet d'esquive: ${dodgeResult.diceRoll}/${dodgeResult.targetNumber} ${dodgeResult.success ? '✓' : '✗'}`, player.id, player.team, {
        diceRoll: dodgeResult.diceRoll,
        targetNumber: dodgeResult.targetNumber,
        success: dodgeResult.success,
        modifiers: dodgeModifiers,
    ...
    if (dodgeResult.success) {
        ...
        next.lastDiceResult = { ...armorResult, success: armorSuccess };
        const armorLogEntry = createLogEntry('dice', `Jet d'armure (Dodge échoué): ${armorResult.diceRoll}/${armorResult.targetNumber} ${armorSuccess ? '✓' : '✗'}`, player.id, player.team, {
            diceRoll: armorResult.diceRoll,
            targetNumber: armorResult.targetNumber,
 ...
 * Gère un blocage
 */
function handleBlock(state, move, rng) {
    ...
    if (!canBlock(state, move.playerId, move.targetId))
        return state;
    ...
    if (diceCount === 1) {
        ...
        const blockLogEntry = createLogEntry('dice', `Blocage: ${diceRoll} → ${blockResult}`, attacker.id, attacker.team, { diceRoll: diceRoll, result: blockResult, offensiveAssists, defensiveAssists });
        ...
        const blockLogEntry = createLogEntry('dice', `Blocage: ${options.join(', ')} (${diceCount} dés)`, attacker.id, attacker.team, { results: options, diceCount, offensiveAssists, defensiveAssists });
        newState.gameLog = [...newState.gameLog, blockLogEntry];
        return {
            ...newState,
            pendingBlock: {
 ...
 */
function handleBlockChoose(state, move, rng) {
    const attacker = state.players.find(p => p.id === move.playerId);
    const target = state.players.find(p => p.id === move.targetId);
    if (!attacker || !target)
        return state;
 ...
 */
function handlePushChoose(state, move) {
    const attacker = state.players.find(p => p.id === move.playerId);
    const target = state.players.find(p => p.id === move.targetId);
    if (!attacker || !target)
        return state;
    ...
    const isValidDirection = state.pendingPushChoice.availableDirections.some(dir => dir.x === move.direction.x && dir.y === move.direction.y);
    if (!isValidDirection)
        return state;
    ...
    newState.players = newState.players.map(p => p.id === target.id ? { ...p, pos: newTargetPos } : p);
    ...
    // Log de la poussée
    const pushLog = createLogEntry('action', `${target.name} repoussé vers (${newTargetPos.x}, ${newTargetPos.y}) par ${attacker.name}`, attacker.id, attacker.team);
 ...
 */
function handleFollowUpChoose(state, move) {
    const attacker = state.players.find(p => p.id === move.playerId);
    const target = state.players.find(p => p.id === move.targetId);
    if (!attacker || !target)
        return state;
    ...
    if (move.followUp) {
        ...
        newState.players = newState.players.map(p => p.id === attacker.id ? { ...p, pos: state.pendingFollowUpChoice.targetOldPosition } : p);
        const followUpLog = createLogEntry('action', `${attacker.name} suit ${target.name} (follow-up)`, attacker.id, attacker.team);
        newState.gameLog = [...newState.gameLog, followUpLog];
    }
    else {
        const noFollowUpLog = createLogEntry('action', `${attacker.name} ne suit pas ${target.name}`, attacker.id, attacker.team);
 ...
 * Gère un blitz
 */
function handleBlitz(state, move, rng) {
    const attacker = state.players.find(p => p.id === move.playerId);
    const target = state.players.find(p => p.id === move.targetId);
    if (!attacker || !target)
        return state;
    ...
    if (needsDodge) {
        ...
        // Log du jet d'esquive
        const dodgeLogEntry = createLogEntry('dice', `Jet d'esquive (Blitz): ${dodgeResult.diceRoll}/${dodgeResult.targetNumber} ${dodgeResult.success ? '✓' : '✗'}`, attacker.id, attacker.team, {
            diceRoll: dodgeResult.diceRoll,
            targetNumber: dodgeResult.targetNumber,
            success: dodgeResult.success,
            modifiers: dodgeModifiers,
        ...
        if (dodgeResult.success) {
            ...
            // Log du jet d'armure
            const armorLogEntry = createLogEntry('dice', `Jet d'armure (Blitz échoué): ${armorResult.diceRoll}/${armorResult.targetNumber} ${armorResult.success ? '✓' : '✗'}`, newState.players[attackerIdx].id, newState.players[attackerIdx].team, {
                diceRoll: armorResult.diceRoll,
                targetNumber: armorResult.targetNumber,
                success: armorResult.success,
            });
            ...
            if (hadBall) {
                ...
                const ballLossLogEntry = createLogEntry('action', `Ballon perdu après échec de blitz`, attacker.id, attacker.team);
    ...
    const blitzLogEntry = createLogEntry('action', `Blitz: mouvement vers (${to.x}, ${to.y}) puis blocage de ${updatedTarget.name}`, attacker.id, attacker.team);
