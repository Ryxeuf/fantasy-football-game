/**
 * S27.8.15 — Module dedie a `handleBlock` (~200 lignes), le plus
 * gros handler de `block-action.ts`. Extrait pour ramener
 * `block-action.ts` sous la cible secondaire DoD `<= 400 lignes`.
 *
 * Le flux complet est preserve :
 *  - detection blitz pendant un mouvement (`canTeamBlitz`) vs blocage
 *    classique (`canBlock`).
 ...
 *      `handleBlockChoose`.
 *
 * Aucune dependance vers `block-action.ts` : `handleBlock` est appele
 * par `handleDumpOffChoose`, `handleMultiBlock`, `resolveFrenzyBlock`,
 * `resolveMultipleBlock` (tous restes dans `block-action.ts`) — la
 * dependance est unidirectionnelle, pas cyclique.
 */
