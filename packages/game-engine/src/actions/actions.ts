...
// S27.8.9 — Tous les helpers de des bruts (`performDodgeRoll`,
// `rollBlockDice`, `blockResultFromRoll`, `rollBlockDiceManyWithRolls`,
// etc.) sont consommes dans les modules extraits (`block-action`,
// `blitz-action`, `move-handlers`, `choice-handlers`,
// `failure-helpers`, `ball-pickup`, `pass-actions`).
...
// S27.8.9 — `calculateOffensiveAssists`, `calculateDefensiveAssists`,
// `calculateBlockDiceCount`, `getBlockDiceChooser` consommes uniquement
// dans `actions/block-action.ts` + `actions/blitz-action.ts`. Restent
// `canBlock` et `canBlitz` car utilises dans `getLegalMoves`.
...
import {
  ...
  shouldAutoEndTurn,
  handlePlayerSwitch,
  // S27.8.9 — `incrementTeamBlitzCount` consomme uniquement dans
  // `actions/blitz-action.ts`. `canTeamBlitz` reste car utilise dans
  // `getLegalMoves` pour gater les options BLITZ.
  canTeamBlitz,
...
// / handleBallPickup. S27.8.9 : `applyPickupFailure` n'est plus
// consomme directement par `actions.ts` (uniquement par les modules
// extraits). On garde `applyRollFailure` car appele par `handleLeap`.
...
// S27.8.9 — `handleBlitz` extrait dans `actions/blitz-action.ts`.
// Appele depuis le dispatcher uniquement.
import { handleBlitz } from './blitz-action';
// S27.8.9 — `canDumpOff` / `getDumpOffReceivers` consommes uniquement
// dans `actions/block-action.ts`. `executeDumpOff` reste utilise par
// `handleDumpOffChoose` ci-dessous.
import { executeDumpOff } from '../mechanics/dump-off';
// S27.8.9 — `checkDauntless` consomme uniquement dans
// `actions/block-action.ts` (Dauntless egalisation des forces).
...
// `actions/choice-handlers.ts` (handlePushChoose).
// S27.8.9 — `getArmBarBonus` consomme uniquement dans
// `actions/move-handlers.ts` + `actions/ball-pickup.ts` +
// `actions/choice-handlers.ts` (toutes les call sites de
// `applyRollFailure` qui passent un bonus armure).
...
// S27.8.9 — `checkFoulAppearance` consomme uniquement dans
// `actions/block-action.ts` + `actions/blitz-action.ts`. Reste les
// checks d'activation utilises dans `applyMove`.
// S27.8.9 — `logInstablePrevention` consomme uniquement dans les
// helpers extraits (special-actions, pass-actions, etc.). On garde les
// checks d'activation utilises dans `applyMove` et
// `canInstablePerformAction` utilise par `getLegalMoves`.
import { checkBoneHead, checkReallyStupid, checkWildAnimal, checkAnimalSavagery, checkTakeRoot, checkBloodlust, canInstablePerformAction } from '../mechanics/negative-traits';
import {
