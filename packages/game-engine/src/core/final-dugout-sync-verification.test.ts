...
describe('Final Dugout Field Synchronization Verification', () => {
  ...
  it('should handle the exact scenario from the user image', () => {
    ...
    const teamDugoutLogic = (allPlayers: any[], placedPlayers: string[], dugout: any) => {
      ...
      const reservePlayers = allPlayers.filter((p) => {
        if (p.team !== dugout.teamId) return false;
        if (placedPlayers.includes(p.id)) return false;
        if (p.pos && p.pos.x >= 0) return false; // CORRECTION
        return true;
      });
  ...
  it('should verify the fix prevents the exact error from the user', () => {
    ...
    const beforeFix = (allPlayers: any[], placedPlayers: string[], dugout: any) => {
      // Logique AVANT la correction (problématique)
      if (!allPlayers || !Array.isArray(allPlayers)) return [];
      if (!placedPlayers || !Array.isArray(placedPlayers)) return [];
      if (!dugout || !dugout.teamId) return [];
    ...
    const afterFix = (allPlayers: any[], placedPlayers: string[], dugout: any) => {
  ...
  it('should handle edge cases correctly', () => {
    const teamDugoutLogic = (allPlayers: any[], placedPlayers: string[], dugout: any) => {
      ...
      return allPlayers.filter((p) => {
        if (p.team !== dugout.teamId) return false;
        if (placedPlayers.includes(p.id)) return false;
        if (p.pos && p.pos.x >= 0) return false;
        return true;
      });
    };
