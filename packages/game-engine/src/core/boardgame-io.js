...
export function toBGIOGame() {
    return {
        ...
        moves: {
            ...
            DODGE: (G, ctx, args) => {
                const rng = makeRNG(String(ctx.turn || 1));
                const s2 = applyMove(G, { type: 'DODGE', playerId: args.playerId, from: args.from, to: args.to }, rng);
                Object.assign(G, s2);
            },
            BLOCK: (G, ctx, args) => {
                const rng = makeRNG(String(ctx.turn || 1));
                const s2 = applyMove(G, { type: 'BLOCK', playerId: args.playerId, targetId: args.targetId }, rng);
                Object.assign(G, s2);
            },
            END_TURN: (G, ctx) => {
                const rng = makeRNG(String(ctx.turn || 1));
