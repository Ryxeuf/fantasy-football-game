...
import { setupPreMatchWithTeams, enterSetupPhase, } from './game-state';
...
describe('Simple Kickoff Logic', () => {
    it('doit permettre le kick-off seulement si les deux équipes ont placé leurs joueurs', () => {
        ...
        currentState = {
            ...currentState,
            players: currentState.players.map(p => p.team === 'A' ? { ...p, pos: { x: 5, y: 3 } } : p),
            preMatch: {
        ...
        currentState = {
            ...currentState,
            players: currentState.players.map(p => p.team === 'B' ? { ...p, pos: { x: 15, y: 3 } } : p)
        };
