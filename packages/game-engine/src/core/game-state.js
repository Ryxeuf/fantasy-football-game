 ...
 */
export function setupPreMatchWithTeams(teamAData, teamBData, teamAName, teamBName) {
    const dugouts = initializeDugouts();
    // Créer les joueurs de l'équipe A
    const teamAPlayers = teamAData.map((tp, index) => ({
        id: `A${tp.number}`,
        team: 'A',
    ...
    const teamBPlayers = teamBData.map((tp, index) => ({
        id: `B${tp.number}`,
        team: 'B',
        pos: { x: -1, y: -1 }, // Position hors terrain
        name: tp.name,
    ...
    const existingState = {
        ...
        // Log du match
        gameLog: [
            createLogEntry('info', `Phase pré-match - ${teamAName} vs ${teamBName} - Les joueurs sont en réserves`),
        ],
    };
    return {
        ...existingState,
        preMatch: {
            phase: 'idle',
            currentCoach: 'A', // À set par backend après coin toss
            legalSetupPositions: [],
            placedPlayers: [],
            kickingTeam: 'B', // Placeholder, set par backend
            receivingTeam: 'A',
        },
    };
}
 ...
 */
export function startMatchFromPreMatch(state) {
    if (state.half !== 0)
        return state; // Déjà démarré
 ...
 */
export function advanceHalfIfNeeded(state) {
    // Si on a dépassé le 8e round, on passe à la mi‑temps suivante ou on termine le match
    if (state.turn > 8) {
        if (state.half === 1) {
            const halftimeLog = createLogEntry('info', `Mi-temps atteinte (8 tours par équipe). Début de la 2e mi-temps`, undefined, undefined);
            return {
                ...state,
                half: 2,
                turn: 1,
            ...
            const endLog = createLogEntry('info', `Fin du match (2e mi-temps terminée)`, undefined, undefined);
            return {
                ...state,
                isTurnover: true,
 ...
 */
export function hasPlayerActed(state, playerId) {
    if (!state.playerActions || typeof state.playerActions.has !== 'function')
        return false; // Guard pour JSON deserialized
 ...
 */
export function canPlayerAct(state, playerId) {
 ...
 */
export function canPlayerMove(state, playerId) {
    const player = state.players.find(p => p.id === playerId);
    if (!player)
        return false;
 ...
 */
export function canPlayerContinueMoving(state, playerId) {
    const player = state.players.find(p => p.id === playerId);
    if (!player)
        return false;
 ...
 */
export function getPlayerAction(state, playerId) {
    if (!state.playerActions || typeof state.playerActions.get !== 'function')
        return undefined;
 ...
 */
export function setPlayerAction(state, playerId, action) {
 ...
 * @returns Nouvel état du jeu
 */
export function clearPlayerActions(state) {
    return {
        ...state,
        playerActions: new Map(),
    };
 ...
 */
export function getTeamBlitzCount(state, team) {
    if (!state.teamBlitzCount || typeof state.teamBlitzCount.get !== 'function')
        return 0;
 ...
 */
export function canTeamBlitz(state, team) {
 ...
 * @returns Nouvel état du jeu
 */
export function incrementTeamBlitzCount(state, team) {
 ...
 * @returns Nouvel état du jeu
 */
export function clearTeamBlitzCounts(state) {
    return {
        ...state,
        teamBlitzCount: new Map(),
    };
 ...
 */
export function shouldEndPlayerTurn(state, playerId) {
...
export function endPlayerTurn(state, playerId) {
    ...
    const player = state.players.find(p => p.id === playerId);
    if (player) {
        const logEntry = createLogEntry('action', `Fin du tour de ${player.name}`, player.id, player.team);
 ...
 * @returns Nouvel état du jeu
 */
export function checkPlayerTurnEnd(state, playerId) {
    const player = state.players.find(p => p.id === playerId);
    if (!player)
        return state;
 ...
 * @returns True si le tour doit se terminer
 */
export function shouldAutoEndTurn(state) {
    ...
    return teamPlayers.every(player => hasPlayerActed(state, player.id) || player.stunned || player.pm <= 0);
}
/**
 ...
 * @returns Nouvel état du jeu
 */
export function handlePlayerSwitch(state, newPlayerId) {
 ...
 * @returns Nouvel état du jeu
 */
export function clearDiceResult(state) {
    return {
        ...state,
        lastDiceResult: undefined,
    };
...
export function enterSetupPhase(state, receivingTeam) {
    if (state.half !== 0 || (state.preMatch.phase !== 'idle' && state.preMatch.phase !== 'setup'))
        return state;
 ...
 */
export function validatePlayerPlacement(state) {
    if (state.preMatch.phase !== 'setup')
        return state;
    ...
    const coachPlayersOnField = state.players.filter(p => p.team === currentCoach && p.pos.x >= 0).length;
    if (coachPlayersOnField !== 11) {
        return state; // Pas assez de joueurs placés
    }
    const logEntry = createLogEntry('action', `Placement validé par l'équipe ${state.teamNames[currentCoach === 'A' ? 'teamA' : 'teamB']} - 11 joueurs placés`);
 ...
 */
export function startKickoffSequence(state) {
    if (state.half !== 0 || state.preMatch.phase !== 'kickoff')
        return state;
    // Créer un log de début de kickoff
    const kickoffLog = createLogEntry('info', `Kickoff - ${state.teamNames.teamA} vs ${state.teamNames.teamB} - Début de la séquence de kickoff`);
    return {
        ...state,
        preMatch: {
            ...state.preMatch,
 ...
 * @returns État de match démarré
 */
export function startMatchFromKickoff(state) {
    if (state.half !== 0 || state.preMatch.phase !== 'kickoff-sequence')
        return state;
    // Créer un log de début de match
    const matchStartLog = createLogEntry('info', `Match commencé - ${state.teamNames.teamA} vs ${state.teamNames.teamB} - Le jeu commence !`);
 ...
 */
export function placeKickoffBall(state, position) {
    ...
    const logEntry = createLogEntry('action', `Ballon placé en position (${position.x}, ${position.y}) pour le kickoff`, undefined, state.preMatch.kickingTeam);
    return {
        ...state,
        preMatch: {
            ...state.preMatch,
 ...
 */
export function calculateKickDeviation(state, rng) {
    ...
    const logEntry = createLogEntry('action', `Déviation du kickoff: D8=${d8} (${direction}), D6=${d6} → Ballon en (${finalPosition.x}, ${finalPosition.y})`, undefined, state.preMatch.kickingTeam);
    return {
        ...state,
        preMatch: {
            ...state.preMatch,
 ...
 */
export function resolveKickoffEvent(state, rng) {
    ...
    const logEntry = createLogEntry('action', `Événement de kickoff: ${dice1}+${dice2}=${total} - ${eventData.event}: ${eventData.description}`, undefined, state.preMatch.kickingTeam);
    return {
        ...state,
        preMatch: {
            ...state.preMatch,
