...
describe('TeamDugout Safety Corrections', () => {
  it('should handle undefined allPlayers gracefully', () => {
    ...
    const safeFilter = (allPlayers: any, filterFn: (p: any) => boolean) => {
  ...
  it('should verify all TeamDugout corrections work together', () => {
    ...
    const teamDugoutSafe = (allPlayers: any, dugout: any, placedPlayers: any) => {
      // Reserve players
      const reservePlayers = allPlayers?.filter(
        (p: any) => p.team === dugout?.teamId && !placedPlayers?.includes(p.id)
      ) || [];
      
      // KO players
      const koPlayers = allPlayers?.filter(
        (p: any) => p.pos?.x === -2 && p.team === dugout?.teamId
      ) || [];
      
      // Stunned players
      const stunnedPlayers = allPlayers?.filter(
        (p: any) => p.stunned && p.team === dugout?.teamId
      ) || [];
      
      // Casualty players
      const casualtyPlayers = allPlayers?.filter(
        (p: any) => p.pos?.x === -3 && p.team === dugout?.teamId
      ) || [];
