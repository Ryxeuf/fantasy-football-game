...
describe('Final Complete Safety Verification', () => {
  it('should handle all TeamDugout error cases', () => {
    ...
    const safeTeamDugout = (allPlayers: any, placedPlayers: any, dugout: any) => {
      ...
      const reservePlayers = allPlayers.filter((p) => {
        ...
        if (p.pos && (p.pos.x === -2 || p.pos.x === -3)) return false;
        return true;
      });
      
      // Filtrer les joueurs KO
      const koPlayers = allPlayers.filter((p) => {
        if (p.pos?.x !== -2) return false;
        if (p.team !== dugout.teamId) return false;
        return true;
      });
