  ...
  describe("Échec de blitz avec jet d'armure", () => {
    it("devrait déclencher un jet de blessure après échec de blitz si l'armure est percée", () => {
      ...
      const testState = {
        ...state,
        players: state.players.map(p => {
          if (p.id === 'A1') return { ...p, pos: { x: 10, y: 7 }, stunned: false, pm: 7, ag: 1 }; // AG très faible
          if (p.id === 'B1') return { ...p, pos: { x: 12, y: 7 }, stunned: false, pm: 8 }; // Adjacent à la destination
          if (p.id === 'B2') return { ...p, pos: { x: 11, y: 6 }, stunned: false, pm: 8 }; // Adjacent à A1
          if (p.id === 'B3') return { ...p, pos: { x: 11, y: 8 }, stunned: false, pm: 8 }; // Adjacent à A1
          if (p.id === 'B4') return { ...p, pos: { x: 10, y: 6 }, stunned: false, pm: 8 }; // Adjacent à A1
          if (p.id === 'B5') return { ...p, pos: { x: 10, y: 8 }, stunned: false, pm: 8 }; // Adjacent à A1
          return p;
        }),
      };
