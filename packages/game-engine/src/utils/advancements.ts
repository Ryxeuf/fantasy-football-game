...
export type AdvancementType = 'primary' | 'secondary' | 'random-primary' | 'random-secondary';
 ...
 */
export function calculateAdvancementsSurcharge(advancements: AdvancementType[]): number {
 ...
 */
export function calculatePlayerCurrentValue(baseCostPo: number, advancements: AdvancementType[]): number {
  return baseCostPo + calculateAdvancementsSurcharge(advancements);
}

export interface PlayerAdvancement {
  skillSlug: string;           // slug de la compétence ajoutée
  type: AdvancementType;       // primaire, secondaire, random-primary ou random-secondary
  isRandom: boolean;           // true si c'était un choix aléatoire
