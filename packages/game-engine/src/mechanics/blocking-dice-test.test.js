...
import { setup, applyMove, makeRNG, rollBlockDice, rollBlockDiceMany, rollBlockDiceManyWithRolls, calculateBlockDiceCount, getBlockDiceChooser, resolveBlockResult, } from '../index';
describe('Tests des dés de blocage - Vérification complète', () => {
    ...
    describe('Vérification des faces de dés', () => {
        ...
        it('devrait avoir la distribution correcte des faces (Push Back sur 2 faces)', () => {
            ...
            const singleFaceFaces = ['PLAYER_DOWN', 'BOTH_DOWN', 'STUMBLE', 'POW'];
        ...
        it('devrait correspondre aux règles officielles de Blood Bowl', () => {
            ...
            const actualFaces = new Set();
            for (let i = 0; i < 100; i++) {
                actualFaces.add(rollBlockDice(rng));
            }
            for (const face of expectedFaces) {
                expect(actualFaces.has(face)).toBe(true);
            }
        });
    });
