 ...
 */
export function canBlock(state, playerId, targetId) {
    const player = state.players.find(p => p.id === playerId);
    const target = state.players.find(p => p.id === targetId);
    if (!player || !target)
        return false;
    // Le joueur doit être debout et non étourdi
    if (player.stunned || player.pm <= 0)
        return false;
    // La cible doit être debout et non étourdie
    if (target.stunned || target.pm <= 0)
        return false;
...
export function canBlitz(state, attackerId, to, targetId) {
    ...
    if (attacker.pm < distance + 1)
        return false; // +1 pour le blocage
 ...
 */
export function calculateOffensiveAssists(state, attacker, target) {
    let assists = 0;
    // Trouver tous les coéquipiers de l'attaquant qui marquent la cible
    const teammates = state.players.filter(p => p.team === attacker.team && p.id !== attacker.id && !p.stunned && p.pm > 0);
 ...
 */
export function calculateDefensiveAssists(state, attacker, target) {
    let assists = 0;
    // Trouver tous les coéquipiers de la cible qui marquent l'attaquant
    const teammates = state.players.filter(p => p.team === target.team && p.id !== target.id && !p.stunned && p.pm > 0);
    for (const teammate of teammates) {
 ...
 */
export function calculateBlockDiceCount(attackerStrength, targetStrength) {
 ...
 */
export function getBlockDiceChooser(attackerStrength, targetStrength) {
 ...
 */
export function getPushDirection(attackerPos, targetPos) {
 ...
 */
export function getPushDirections(attackerPos, targetPos) {
    const dx = targetPos.x - attackerPos.x;
    const dy = targetPos.y - attackerPos.y;
 ...
 */
export function handlePushWithChoice(state, attacker, target, blockResult) {
    ...
    if (availableDirections.length === 0) {
        ...
        const noPushLog = createLogEntry('action', `${target.name} ne peut pas être repoussé (aucune case libre)`, attacker.id, attacker.team);
        ...
        const pushLog = createLogEntry('action', `${target.name} repoussé vers (${newTargetPos.x}, ${newTargetPos.y})`, attacker.id, attacker.team);
        ...
        const choiceLog = createLogEntry('action', `${attacker.name} doit choisir la direction de poussée pour ${target.name}`, attacker.id, attacker.team);
 ...
 */
export function resolveBlockResult(state, blockResult, rng) {
    ...
    const blockLogEntry = createLogEntry('dice', `Blocage: ${blockResult.result} (${blockResult.totalStrength} vs ${blockResult.targetStrength})`, attacker.id, attacker.team, {
 ...
 */
function handlePlayerDown(state, attacker, target, rng) {
    ...
    const attackerDownLog = createLogEntry('action', `${attacker.name} est mis au sol par ${target.name}`, attacker.id, attacker.team);
    ...
    // Log du jet d'armure
    const attackerArmorLog = createLogEntry('dice', `Jet d'armure: ${attackerArmorResult.diceRoll}/${attackerArmorResult.targetNumber} ${attackerArmorResult.success ? '✓' : '✗'}`, attacker.id, attacker.team, {
 ...
 */
function handleBothDown(state, attacker, target, rng) {
    // Les deux joueurs sont mis au sol
    state.players = state.players.map(p => p.id === attacker.id || p.id === target.id ? { ...p, stunned: true } : p);
    state.isTurnover = true;
    // Log de la chute des deux joueurs
    const bothDownLog = createLogEntry('action', `${attacker.name} et ${target.name} tombent tous les deux`, attacker.id, attacker.team);
    ...
    const attackerArmorLog = createLogEntry('dice', `Jet d'armure attaquant: ${attackerArmorResult.diceRoll}/${attackerArmorResult.targetNumber} ${attackerArmorResult.success ? '✓' : '✗'}`, attacker.id, attacker.team, {
        diceRoll: attackerArmorResult.diceRoll,
        targetNumber: attackerArmorResult.targetNumber,
        success: attackerArmorResult.success,
    });
    const targetArmorLog = createLogEntry('dice', `Jet d'armure cible: ${targetArmorResult.diceRoll}/${targetArmorResult.targetNumber} ${targetArmorResult.success ? '✓' : '✗'}`, target.id, target.team, {
 ...
 */
function handlePushBack(state, attacker, target) {
    ...
    if (availableDirections.length === 0) {
        ...
        const noPushLog = createLogEntry('action', `${target.name} ne peut pas être repoussé (toutes les directions bloquées)`, attacker.id, attacker.team);
 ...
 */
function handleStumble(state, attacker, target, rng) {
    ...
    if (target.skills.includes('Dodge')) {
        ...
        const stumbleDownLog = createLogEntry('action', `${target.name} est mis au sol par ${attacker.name}`, attacker.id, attacker.team);
        ...
        // Log du jet d'armure
        const targetArmorLog = createLogEntry('dice', `Jet d'armure: ${targetArmorResult.diceRoll}/${targetArmorResult.targetNumber} ${targetArmorResult.success ? '✓' : '✗'}`, target.id, target.team, {
            diceRoll: targetArmorResult.diceRoll,
            targetNumber: targetArmorResult.targetNumber,
            success: targetArmorResult.success,
        });
        ...
        if (target.hasBall) {
            pushResult.players = pushResult.players.map(p => p.id === target.id ? { ...p, hasBall: false } : p);
 ...
 */
function handlePow(state, attacker, target, rng) {
    // La cible est repoussée puis mise au sol
    state.players = state.players.map(p => (p.id === target.id ? { ...p, stunned: true } : p));
    state.isTurnover = true;
    // Log de la mise au sol
    const powLog = createLogEntry('action', `${target.name} est mis au sol par ${attacker.name} (POW!)`, attacker.id, attacker.team);
