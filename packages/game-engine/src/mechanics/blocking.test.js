...
import { setup, applyMove, makeRNG, canBlock, calculateOffensiveAssists, calculateDefensiveAssists, performBlockRoll, resolveBlockResult, getAdjacentOpponents, isAdjacent, getPushDirection, isPositionOccupied, } from '../index';
describe('Système de blocage', () => {
        ...
        it('devrait détecter les joueurs adjacents', () => {
            ...
            const newState = {
                ...
                players: state.players.map(p => {
                    if (p.id === 'A1')
                        return { ...p, pos: { x: 10, y: 7 } };
                    if (p.id === 'B1')
                        return { ...p, pos: { x: 11, y: 7 } };
                    return p;
                }),
            };
            ...
            const newState = {
                ...
                players: state.players.map(p => {
                    if (p.id === 'A1')
                        return { ...p, pos: { x: 10, y: 7 }, stunned: false, pm: 7 };
                    if (p.id === 'A2')
                        return { ...p, pos: { x: 11, y: 7 }, stunned: false, pm: 6 };
                    return p;
                }),
            };
            ...
            const newState = {
                ...
                players: state.players.map(p => {
                    if (p.id === 'A1')
                        return { ...p, pos: { x: 10, y: 7 }, stunned: false, pm: 7 };
                    if (p.id === 'B1')
                        return { ...p, pos: { x: 11, y: 7 }, stunned: true, pm: 8 };
                    return p;
                }),
            };
            expect(canBlock(newState, 'A1', 'B1')).toBe(false);
            ...
            const newState = {
                ...
                players: state.players.map(p => {
                    if (p.id === 'A1')
                        return { ...p, pos: { x: 10, y: 7 } };
                    if (p.id === 'A2')
                        return { ...p, pos: { x: 11, y: 8 }, stunned: false, pm: 6 }; // Adjacent à la cible
                    if (p.id === 'B1')
                        return { ...p, pos: { x: 11, y: 7 } };
                    if (p.id === 'B2')
                        return { ...p, pos: { x: 20, y: 20 } }; // Loin pour éviter les conflits
                    return p;
                }),
            };
            ...
            const newState = {
                ...
                players: state.players.map(p => {
                    ...
                    if (p.id === 'B1')
                        return { ...p, pos: { x: 11, y: 7 } };
                    if (p.id === 'B2')
                        return { ...p, pos: { x: 10, y: 8 }, stunned: false, pm: 6 }; // Adjacent à l'attaquant
                    return p;
                }),
            };
            const attacker = newState.players.find(p => p.id === 'A1');
    ...
    describe('Fonctions utilitaires', () => {
        it('devrait calculer la direction de poussée', () => {
            const attackerPos = { x: 10, y: 7 };
            const targetPos = { x: 11, y: 7 };
        ...
        it('devrait vérifier si une position est occupée', () => {
            const occupiedPos = { x: 11, y: 7 }; // Position d'un joueur (A1)
            const freePos = { x: 5, y: 5 }; // Position libre
    ...
    describe('Intégration dans le jeu', () => {
        ...
        it('devrait exécuter une action de blocage', () => {
            ...
            const resultState = applyMove(newState, { type: 'BLOCK', playerId: 'A1', targetId: 'B1' }, rng);
