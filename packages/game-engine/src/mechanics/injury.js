 ...
 */
export function performInjuryRoll(state, player, rng) {
    const newState = structuredClone(state);
    // Jet de 2D6 pour la blessure
    const injuryRoll = Math.floor(rng() * 6) + 1 + Math.floor(rng() * 6) + 1;
    // Log du jet de blessure
    const injuryLog = createLogEntry('dice', `Jet de blessure: ${injuryRoll}`, player.id, player.team, { injuryRoll });
 ...
 */
function handleStunned(state, player) {
    // Le joueur reste sur le terrain mais devient sonné
    state.players = state.players.map(p => p.id === player.id ? { ...p, stunned: true, state: 'stunned' } : p);
 ...
 */
function handleKnockedOut(state, player) {
    // Le joueur va en zone KO
    const newState = movePlayerToDugoutZone(state, player.id, 'knockedOut', player.team);
    const koLog = createLogEntry('action', `${player.name} est KO et retiré du terrain`, player.id, player.team);
 ...
 */
function handleCasualty(state, player, rng) {
    ...
    const casualtyLog = createLogEntry('dice', `Jet de casualty: ${casualtyRoll}`, player.id, player.team, { casualtyRoll });
    newState.gameLog = [...newState.gameLog, casualtyLog];
    // Déterminer le type de blessure
    if (casualtyRoll <= 6) {
        // Badly Hurt - manque le reste du match
        const badlyHurtLog = createLogEntry('action', `${player.name} est gravement blessé - manque le reste du match`, player.id, player.team);
        newState.gameLog = [...newState.gameLog, badlyHurtLog];
    }
    else if (casualtyRoll <= 9) {
        // Seriously Hurt - MNG (Miss Next Game)
        const seriouslyHurtLog = createLogEntry('action', `${player.name} est sérieusement blessé - manquera le prochain match`, player.id, player.team);
        newState.gameLog = [...newState.gameLog, seriouslyHurtLog];
    }
    else if (casualtyRoll <= 12) {
        // Serious Injury - NI + MNG
        const seriousInjuryLog = createLogEntry('action', `${player.name} a une blessure sérieuse - blessure gênante + manquera le prochain match`, player.id, player.team);
        newState.gameLog = [...newState.gameLog, seriousInjuryLog];
    }
    else if (casualtyRoll <= 14) {
        // Lasting Injury - réduction de caractéristique + MNG
        const lastingInjuryLog = createLogEntry('action', `${player.name} a une blessure permanente - réduction de caractéristique + manquera le prochain match`, player.id, player.team);
 ...
 */
export function handleSentOff(state, player) {
    const newState = movePlayerToDugoutZone(state, player.id, 'sentOff', player.team);
    const sentOffLog = createLogEntry('action', `${player.name} est exclu par l'arbitre`, player.id, player.team);
 ...
 */
export function handleInjuryByCrowd(state, player, rng) {
