...
export const SKILLS_BY_SLUG = SKILLS_DEFINITIONS.reduce((acc, skill) => {
    acc[skill.slug] = skill;
    return acc;
}, {});
// Index par nom français
export const SKILLS_BY_NAME_FR = SKILLS_DEFINITIONS.reduce((acc, skill) => {
    acc[skill.nameFr] = skill;
    return acc;
}, {});
// Index par nom anglais
export const SKILLS_BY_NAME_EN = SKILLS_DEFINITIONS.reduce((acc, skill) => {
 ...
 */
export function formatSkillSlugs(slugs) {
    return slugs.join(",");
}
/**
 ...
 */
export function convertNamesToSlugs(skillsString) {
 ...
 */
export function getDisplayNames(skillsString, language = "fr") {
    const slugs = parseSkillSlugs(skillsString);
    return slugs.map(slug => {
        const skill = getSkillBySlug(slug);
        if (!skill)
            return slug; // Si pas trouvé, retourner le slug
