...
describe('Test de stress - Simulation intensive', () => {
  it('devrait simuler 20 tours avec des actions aléatoires', () => {
    ...
    const getRandomAction = (playerId: string): Move | null => {
      ...
      for (let x = Math.max(0, player.pos.x - 2); x <= Math.min(gameState.width - 1, player.pos.x + 2); x++) {
        for (let y = Math.max(0, player.pos.y - 2); y <= Math.min(gameState.height - 1, player.pos.y + 2); y++) {
