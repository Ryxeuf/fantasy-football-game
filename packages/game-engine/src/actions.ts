...
import { GameState, Move, Player, Position, TeamId, RNG } from './types';
import { inBounds, samePos, isAdjacent, requiresDodgeRoll, calculateDodgeModifiers, calculatePickupModifiers } from './movement';
import { performDodgeRoll, performPickupRoll, performArmorRoll, rollBlockDice, rollBlockDiceManyWithRolls } from './dice';
...
import { 
  canBlock, canBlitz, calculateOffensiveAssists, calculateDefensiveAssists, 
  calculateBlockDiceCount, getBlockDiceChooser, resolveBlockResult, canTeamBlitz 
} from './blocking';
import { 
  hasPlayerActed, canPlayerMove, canPlayerContinueMoving, setPlayerAction, 
  checkPlayerTurnEnd, shouldAutoEndTurn, handlePlayerSwitch, getPlayerAction,
  incrementTeamBlitzCount, clearPlayerActions, clearTeamBlitzCounts, advanceHalfIfNeeded
} from './game-state';
 ...
 */
function handleMove(state: GameState, move: { type: "MOVE"; playerId: string; to: Position }, rng: RNG): GameState {
 ...
 */
function handleDodgeRoll(state: GameState, player: Player, from: Position, to: Position, rng: RNG, idx: number): GameState {
 ...
 */
function handleNormalMove(state: GameState, player: Player, from: Position, to: Position, rng: RNG, idx: number): GameState {
 ...
 */
function handleDodge(state: GameState, move: { type: "DODGE"; playerId: string; from: Position; to: Position }, rng: RNG): GameState {
 ...
 * Gère un blocage
 */
function handleBlock(state: GameState, move: { type: "BLOCK"; playerId: string; targetId: string }, rng: RNG): GameState {
 ...
 */
function handleBlockChoose(state: GameState, move: { type: "BLOCK_CHOOSE"; playerId: string; targetId: string; result: any }, rng: RNG): GameState {
  const attacker = state.players.find(p => p.id === move.playerId);
  const target = state.players.find(p => p.id === move.targetId);
  if (!attacker || !target) return state;
 ...
 */
function handlePushChoose(state: GameState, move: { type: "PUSH_CHOOSE"; playerId: string; targetId: string; direction: Position }): GameState {
  const attacker = state.players.find(p => p.id === move.playerId);
  const target = state.players.find(p => p.id === move.targetId);
  if (!attacker || !target) return state;
 ...
 */
function handleFollowUpChoose(state: GameState, move: { type: "FOLLOW_UP_CHOOSE"; playerId: string; targetId: string; followUp: boolean }): GameState {
  const attacker = state.players.find(p => p.id === move.playerId);
  const target = state.players.find(p => p.id === move.targetId);
  if (!attacker || !target) return state;
 ...
 * Gère un blitz
 */
function handleBlitz(state: GameState, move: { type: "BLITZ"; playerId: string; to: Position; targetId: string }, rng: RNG): GameState {
  const attacker = state.players.find(p => p.id === move.playerId);
  const target = state.players.find(p => p.id === move.targetId);
