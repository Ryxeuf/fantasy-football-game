...
describe('Simulation complète de jeu - 4 tours', () => {
  it('devrait simuler 4 tours complets avec toutes les actions possibles', () => {
    ...
    const getLegalMoves = (playerId: string) => {
      ...
      // Blocages
      const adjacentPlayers = gameState.players.filter(p => 
        p.team !== player.team && 
    ...
    const executeAction = (move: Move): GameState => {
      ...
      while (currentState.pendingPushChoice) {
        ...
        logAction(`Choisit direction de poussée (${chosenDirection.x}, ${chosenDirection.y})`, currentState.pendingPushChoice.attackerId);
      ...
      while (currentState.pendingFollowUpChoice) {
        ...
        logAction(followUp ? 'Suit le joueur poussé' : 'Ne suit pas le joueur poussé', currentState.pendingFollowUpChoice.attackerId);
    ...
    for (let turn = 1; turn <= 4; turn++) {
      ...
      if (touchdowns.length > 0) {
        logAction('TOUCHDOWN!', touchdowns[touchdowns.length - 1].playerId);
      }
    }
  ...
  it('devrait tester les mouvements et changements de tour', () => {
    ...
    if (playerB1) {
      const move2: Move = { type: 'MOVE', playerId: 'B1', to: { x: playerB1.pos.x + 1, y: playerB1.pos.y } };
