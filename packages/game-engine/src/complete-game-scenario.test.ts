...
describe('Scénario de jeu complet - Actions spécifiques', () => {
  it('devrait simuler un scénario de jeu réaliste avec blocages et follow-ups', () => {
    ...
    const executeAction = (move: Move): GameState => {
      ...
      while (currentState.pendingBlock) {
        ...
        log(`${currentState.players.find(p => p.id === currentState.pendingBlock!.attackerId)?.name} choisit ${chosenResult}`);
      ...
      while (currentState.pendingPushChoice) {
        ...
        log(`${currentState.players.find(p => p.id === currentState.pendingPushChoice!.attackerId)?.name} choisit direction (${chosenDirection.x}, ${chosenDirection.y})`);
      ...
      while (currentState.pendingFollowUpChoice) {
        ...
        log(`${currentState.players.find(p => p.id === currentState.pendingFollowUpChoice!.attackerId)?.name} ${followUp ? 'suit' : 'ne suit pas'}`);
  ...
  it('devrait tester un scénario avec turnovers et récupération', () => {
    ...
    const blockLogs = result.gameLog.filter(log => 
