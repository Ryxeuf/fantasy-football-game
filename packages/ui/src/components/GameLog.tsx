          ...
          displayEntries.map((entry) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p
                    className={`text-sm font-medium ${entry.type === "score" ? "text-yellow-900 font-bold" : "text-gray-900"}`}
                  >
                    {entry.message}
                  </p>
                ...
                {entry.details && (
                  <div className="mt-1 text-xs text-gray-500">
                    {entry.type === "dice" &&
                      (entry as any).details?.diceRoll && (
                        <span>
                          ...
                          {(entry as any).details.modifiers &&
                            (entry as any).details.modifiers !== 0 && (
                              <span>
                                {" "}
                                (mod:{" "}
                                {(entry as any).details.modifiers > 0
                    ...
                    {entry.type === "score" &&
                      (entry as any).details?.score && (
                        <span className="block mt-1 text-yellow-800">
                          Score: {(entry as any).details.score.teamA} -{" "}
                          {(entry as any).details.score.teamB}
                        </span>
                      )}
                  </div>
                )}
