...
import { useDiceNotifications } from "./DiceNotification";
import {
...
export const DiceNotificationDemo: React.FC = () => {
  const { showDiceResult, showBlockDiceResult, showCustomDiceResult } =
  ...
  React.useEffect(() => {
    setDiceNotificationCallback(
      (playerName: string, diceResult: DiceResult) => {
        showDiceResult(playerName, diceResult);
      },
    );

    setBlockDiceNotificationCallback(
      (playerName: string, blockResult: BlockResult) => {
  ...
  return (
    <div className="p-6 bg-gray-50 rounded-lg">
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          Cliquez sur les boutons pour tester les différents types de
          notifications de dés.
        </p>
