...
interface ActionPickerPopupProps {
  ...
  onPick: (action: 'MOVE' | 'BLOCK' | 'BLITZ' | 'PASS' | 'HANDOFF' | 'FOUL') => void;
  onClose: () => void;
}

export default function ActionPickerPopup({ playerName, available, onPick, onClose }: ActionPickerPopupProps) {
  const actions = available;
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        <h3 className="text-xl font-bold mb-2 text-center">Choisir l'action</h3>
        <div className="text-sm text-gray-600 text-center mb-4">{playerName}</div>
...
function label(a: ActionPickerPopupProps['available'][number]) {
  switch (a) {
    case 'MOVE': return 'Mouvement';
    case 'BLOCK': return 'Blocage';
    case 'BLITZ': return 'Blitz';
    case 'PASS': return 'Passe';
    case 'HANDOFF': return 'Transmission';
