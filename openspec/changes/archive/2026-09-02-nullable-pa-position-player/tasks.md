...
## 3. Validation / API
- [x] 3.1 `apps/server/src/schemas/admin-data.schemas.ts` (~112-126) : champ
      `pa` optionnel/nullable avec `preprocess` `"" | undefined → null`, sinon
      `Number`. Jamais `NaN`.
- [x] 3.2 `apps/server/src/routes/admin-data.ts` : remplacer
      `pa: parseInt(formData.get("pa"))` par la valeur Zod normalisee
      (`number | null`) cote create ET update position.
- [x] 3.3 Verifier la serialisation de lecture (`admin-data.ts:645,928`) :
      `pa` peut etre `null`, pas de `?? 0`.

## 4. Propagation Position → TeamPlayer (pas de coercition null→0)
- [x] 4.1 `team-create-from-roster-handler.ts:351` : `pa: t.pa` reste
      `number | null` (pas de `Number()`/`?? 0`).
- [x] 4.2 `team-purchase-handler.ts:187`, `utils/roster-helpers.ts:122,183`,
...
- [x] 5.1 `apps/web/app/teams/[slug]/TeamDetailClient.tsx:436,525` :
      `{position.pa}` → `{position.pa != null ? `${position.pa}+` : "—"}`
      (respecter le glyphe local du composant).
- [x] 5.2 `apps/web/app/me/teams/[id]/page.tsx:476,524` : `{p.pa}` → garde.
- [x] 5.3 `apps/web/app/admin/data/rosters/[id]/page.tsx:562` : `{position.pa}`
      dans la chaine `ma/st/ag/pa/av` → `${position.pa ?? "-"}`.
- [x] 5.4 `apps/web/app/admin/users/page.tsx:1287`,
      `apps/web/app/admin/teams/page.tsx:563` : `{player.pa}` → garde.
- [x] 5.5 `apps/web/app/teams/position-detail-structured-data.ts:39` et
...
- [x] 5.6 Sites **deja gardes** (`pa || "-"`, `pa ?? "-"`, `pa ? ... : "—"`,
      `pa !== null && ...`) : verifier qu'ils restent corrects, ne pas toucher.
      (StarPlayerCard, exportPDF, r/[token], comparer, og-image-content, etc.)
- [x] 5.7 `grep -rn '\{[a-z]*\.pa\}' apps/web/app` final : zero rendu brut
      restant non garde.

## 6. Donnees
- [x] 6.1 `data/positions-update.ts:673` : Cingle Gobelin `pa: 6` → `pa: null`.
- [x] 6.2 Verifier les autres sources de positions (seed.ts:353 lit la def) ;
      tout poste sans-PA identifie → `null`.
- [x] 6.3 Reseed local (dans le conteneur) et controler l'affichage `-` du
      Cingle sur la page roster + le catalogue admin.

## 7. Tests & cloture
- [x] 7.1 (TDD, ecrit en premier) Test unite format/affichage : `pa=null → "-"`,
      `pa=0 → "-"`, `pa=2 → "2+"` ; jamais `"null+"`/`"NaN+"`.
- [x] 7.2 Test integration API : create/update position PA vide → `pa: null`
      persiste et relu.
- [x] 7.3 Test non-regression chaine : equipe creee depuis une position
      `pa=null` → `TeamPlayer.pa === null` → rendu `-`.
- [x] 7.4 Suites vertes : server + web (`vitest run`), `tsc` exit 0, prettier
      `--check` clean sur les fichiers touches. Coverage 80%+ maintenu.
- [x] 7.5 `/opsx:sync` (delta-spec → specs principales) puis `/opsx:archive`
